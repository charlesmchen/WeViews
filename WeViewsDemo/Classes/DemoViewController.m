...
- (UIView*) makeHeader:(NSString*) title {
...
- (void) updatePalette {
//    if (paletteHidden) {
//        canvas.stretchWeight = 1.0f;
//        palette.stretchWeight = 0;
//        palette.hidden = YES;
//    } else {
//#define IPAD_LANDSCAPE_SCREEN_WIDTH 1024
//#define IPAD_LANDSCAPE_CANVAS_WIDTH 753
//        canvas.stretchWeight = IPAD_LANDSCAPE_CANVAS_WIDTH;
//        palette.stretchWeight = IPAD_LANDSCAPE_SCREEN_WIDTH - IPAD_LANDSCAPE_CANVAS_WIDTH;
//        palette.hidden = NO;
//    }
...
//    parent.debugLayout = YES;   
    [parent layoutContents];
//    parent.debugLayout = NO;   
}
...
- (void) loadView {        
    UIView* wrapper = [[[UIView alloc] init] autorelease];
    ...
    [statusBar addHorizontalFill:[NSArray arrayWithObjects:
                         ...
                         [WeSpacer create],
                         [WeViews createUIImageView:@"ios_status_bar_right"],
                         nil]];
    [statusBar addClickSelector:@selector(togglePalette)
                         target:self];
    ...
    [[[footer addHorizontal:[NSArray arrayWithObjects:
                         [[WeViews createImageButton:@"footer_save_button"]
                          ...
                          target:self],
                         [[WeViews createImageButton:@"footer_load_button"]
                          addClickSelector:@selector(handleLoad)
                          target:self],
//                       [[WeViews createImageButton:@"footer_code_button"]
//                        addClickTarget:self
//                        selector:@selector(handleCode)],
                         nil]]
      withSpacing:15]
     withMargin:5];
    
    self.windowModel = [WindowModel create];
    
    self.canvas = [Canvas create:windowModel];
    
    ViewHierarchyTree* viewHierarchyTree = [ViewHierarchyTree create:nil
                                                         windowModel:windowModel];
    
    SelectionView* selectionView = [SelectionView create:windowModel];
    selectionView.stretchWeight = 1.0f;
    
    AddViewView* addViewView = [AddViewView create:windowModel];
    addViewView.stretchWeight = 1.0f;
...
- (void) createDefaultContent {
    ...
    [[[panel addVerticalFill:[NSArray arrayWithObjects:
      ...
      withSpacing:5]
     withMargin:25];
    panel.frame = CGRectMake(100, 120, 130, 140);
    [windowModel.pseudoRoot addSubview:panel];
}
...
- (void) viewDidLoad {
    ...
    [windowModel setSelection:windowModel.pseudoRoot];
}
...
- (void) didRotateFromInterfaceOrientation:(UIInterfaceOrientation) fromInterfaceOrientation {
    ...
    //          FormatCGRect(self.view.frame));
    //    NSLog(@"sizeThatFits:current: %@",
    //          FormatCGSize([self.view sizeThatFits:self.view.frame.size]));
    //    NSLog(@"sizeThatFits:zero: %@",
    //          FormatCGSize([self.view sizeThatFits:CGSizeZero]));
}
