...
+ (HandlerGroup*) create {
    return [self create:[NSArray array]];
}

+ (HandlerGroup*) create :(NSArray*) handlers {
    HandlerGroup* result = [[[HandlerGroup alloc] init] autorelease];
    result.handlers = [NSMutableArray arrayWithArray:handlers];
    return result;
}
...
- (void) fireEvent {
    for (id<IHandler> handler in handlers) {
        [handler fireEvent];
    }
}
