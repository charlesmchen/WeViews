...
id getSingleNotificationParam(NSNotification* notification) {
	CFDictionaryRef userInfo = (CFDictionaryRef) notification.userInfo;
	const id param = (const id) CFDictionaryGetValue(userInfo, notification.name);
	return param;
}
...
- (void) sendSingleParamNotification:(NSString*) name
                                    value:(id) value {
    sendSingleParamNotification(name, value);
}

- (id) getSingleNotificationParam:(NSNotification*) notification {
    return getSingleNotificationParam(notification);
}

- (void) addSimpleNotificationListener:(NSString*) name
                              listener:(id) listener
                              selector:(SEL) selector {
    addSimpleNotificationListener(name, listener, selector);
}

- (void) removeSimpleNotificationListener:(id) listener {
    removeSimpleNotificationListener(listener);
}
...
@implementation SimpleNotificationHandler
...
+ (SimpleNotificationHandler*) create :(NSString*) name {
    SimpleNotificationHandler* result = [[[SimpleNotificationHandler alloc] init] autorelease];
    result.name = name;
    return result;
}
