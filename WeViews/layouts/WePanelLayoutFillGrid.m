...
@implementation WePanelLayoutFillGrid

+ (WePanelLayoutFillGrid*) createWithColumnCount:(int) columnCount
                                     uniformGrid:(BOOL) uniformGrid {
    WePanelLayoutFillGrid* result = [[[WePanelLayoutFillGrid alloc] init] autorelease];
    result.columnCount = columnCount;
    result.uniformGrid = uniformGrid;
    return result;
}

+ (WePanelLayoutFillGrid*) createWithRowCount:(int) rowCount
                                  uniformGrid:(BOOL) uniformGrid {
    WePanelLayoutFillGrid* result = [[[WePanelLayoutFillGrid alloc] init] autorelease];
...
//    int rowCount = gridInfo.rowCount;
    ...
    for (int i=0; i < gridInfo.columnCount; i++) {
        int extra = remainderWidth / remainderCount;
        remainderWidth -= extra;
        remainderCount--;
        columnLefts[i] = left;
    ...
    for (int i=0; i < gridInfo.rowCount; i++) {
        int extra = remainderHeight / remainderCount;
        remainderHeight -= extra;
        remainderCount--;
        rowTops[i] = top;
        rowHeights[i] = gridInfo.rowHeights[i] + extra;
        top += rowHeights[i] + layer.spacing;
    }
