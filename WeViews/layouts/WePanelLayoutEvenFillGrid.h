...
/**
 * The "even fill" grid layout.
 *
 * All columns have equal width, all rows have equal height.
 * Panel space is divided equally among all cells.
 *
 * "Natural" size of the contents is _not_ based on the views in
 * the layer.
 * Set cell size to make sure a minimum cell size applies.
 *
 ...
 */
@interface WePanelLayoutEvenFillGrid : WePanelLayoutGridBase

//@property (nonatomic, assign) GridContentMode cellMode; // Not implemented.

/**
 * The ideal cellSize used when calculating the "natural" size in 
 * [WePanelLayoutEvenFillGrid sizeThatFits:layer:]
 */
@property (nonatomic, assign) CGSize cellSizeHint; // optional.  if not set, cell sizes is calculated by dividing layer size equally.
 ...
 * @return A WePanelLayoutEvenFillGrid.
 */
