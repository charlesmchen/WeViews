...
- (id) init {
    ...
    spacing = 0;
    touchMode = LAYER_TOUCH_MODE_ALWAYS;
...
+ (WePanelLayer*) create:(NSArray*) views
    ...
    WePanelLayer* result = [[[WePanelLayer alloc] init] autorelease];
...
- (WePanelLayer*) addView:(UIView*) value {
    NSMutableArray* result = [NSMutableArray arrayWithArray:views];
...
- (BOOL) containsView:(UIView*) value {
    return [views containsObject:value];
}

- (BOOL) debugLayout {
    return panel.debugLayout;
}
