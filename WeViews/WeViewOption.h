...
/**
 * Binds together a label and a handler, for use in any
 * control which presents the user with a list of labeled
 * options.
 */
@interface WeViewOption : WeBaseObject
 ...
 * The handler.
 */
@property (nonatomic, retain) id<IHandler> handler;

/**
 * An optional index property for use by the widget displaying this option.
 */
@property (nonatomic, assign) int customIndex;
 ...
 * @return A WeViewOption.
 */
+ (WeViewOption*) create:(NSString*) label
                  handler:(id<IHandler>) handler;
