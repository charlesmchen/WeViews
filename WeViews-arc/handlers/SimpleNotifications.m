...
id getSingleNotificationParam(NSNotification* notification) {
    ...
    return [userInfo valueForKey:notification.name];
}
...
+ (SimpleNotificationHandler*) create :(NSString*) name {
    SimpleNotificationHandler* result = [[SimpleNotificationHandler alloc] init];
    result.name = name;
    return result;
}
...
- (void) fireEvent {
    if (_value == nil) {
        sendSimpleNotification(_name);
    } else {
        sendSingleParamNotification(_name, _value);
    }
}
