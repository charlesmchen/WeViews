...
+ (URLHandler*) create :(NSString*) url {
    URLHandler* result = [[URLHandler alloc] init];
    result.url = [NSURL URLWithString:url];
    result.isLocalFile = NO;
    return result;
}

+ (URLHandler*) createWithLocalFile:(NSString*) path
                               view:(UIView*) view {
    URLHandler* result = [[URLHandler alloc] init];
    result.url = [NSURL fileURLWithPath:path];
    result.view = view;
    result.isLocalFile = YES;
    return result;
}
...
- (UIView*) findUIWindow :(UIView*) value {
...
// TODO: move this out of the public library and into a category.
// TODO: should be modified to observe interface orientation
// notifications and to handle them properly.
/**
 * Opens a text file in a dialog.
 */
- (void) openFileUrl {
    self.view = [self findUIWindow:_view];

    CGRect frame = [_view frame];
    frame.origin = CGPointZero;
    self.dialog = [[UIView alloc] initWithFrame:frame];
    _dialog.opaque = YES;
    _dialog.backgroundColor = [UIColor blackColor];

#define DIALOG_BUTTON_H_PADDING 64
#define DIALOG_BUTTON_V_PADDING 4
#define DIALOG_WEB_VIEW_PADDING 12
#ifndef L10N_DONE_BUTTON
#define L10N_DONE_BUTTON @"Done"
#endif
    UIButton *button = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [button setTitle:L10N_DONE_BUTTON forState:UIControlStateNormal];
    // TODO: HORROR:
    [button sizeToFit];
    int buttonHeight = button.frame.size.height;
//    int buttonHeight = 48;
    button.frame = CGRectInset(CGRectMake(0,
                                          frame.size.height - buttonHeight,
                                          frame.size.width,
                                          buttonHeight),
                               DIALOG_BUTTON_H_PADDING,
                               DIALOG_BUTTON_V_PADDING);
    [button addTarget:self
               action:@selector(closeDialog)
     forControlEvents:UIControlEventTouchUpInside];
    [_dialog addSubview:button];

    UIWebView *webView = [[UIWebView alloc] initWithFrame:CGRectInset(CGRectMake(0, 0,
                                                                                  frame.size.width,
                                                                                  frame.size.height - buttonHeight),
                                                                       DIALOG_WEB_VIEW_PADDING,
                                                                       DIALOG_WEB_VIEW_PADDING)];
    NSURLRequest *req = [NSURLRequest requestWithURL:_url];
    [webView loadRequest:req];
    [_dialog addSubview:webView];
    [_view addSubview:_dialog];
}
