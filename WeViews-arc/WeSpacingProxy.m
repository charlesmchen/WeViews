...
+ (WeSpacingProxy*) create:(UIView*) view {
    WeSpacingProxy* result = [[WeSpacingProxy alloc] init];
    result.view = view;
...
- (CGSize) sizeThatFits :(CGSize) value {
    return [_view sizeThatFits:value];
}

- (CGFloat) stretchWeight {
    if ([_view respondsToSelector:@selector(stretchWeight)]) {
        return [((id<IWeView>) _view) stretchWeight];
    }
    return 0;
}
