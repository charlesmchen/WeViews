...
@implementation WeCustomImageView

- (void) initDefaults {
    self.backgroundColor = [UIColor clearColor];
    self.opaque = YES;
    self.stretchWeight = 0;
    _mode = IMAGE_LAYOUT_MODE_STRETCH;
...
+ (WeCustomImageView*) create:(UIImage*) image
    ...
    WeCustomImageView* result = [[WeCustomImageView alloc] init];
    result.mode = mode;
...
+ (WeCustomImageView*) create:(NSString*) imageName {
    return [self createStretch:imageName];
}

- (CGSize) sizeThatFits:(CGSize) size {
    CGSize result = _image.size;
...
- (void) setMode:(ImageLayoutMode) value {
    _mode = value;
    [self setNeedsDisplay];
}
    ...
    switch (_mode) {
        ...
        case IMAGE_LAYOUT_MODE_FIT_NATURAL: {
            ...
            if ((imageSize.width <= viewFrame.size.width) &&
                ...
                // If _image is smaller than view frame, don't stretch it larger than
                // its natural size.
                imageRect = alignSizeWithinRect(imageSize, viewFrame, _hAlign, _vAlign);
        ...
        case IMAGE_LAYOUT_MODE_FIT: {
            ...
            imageRect = alignSizeWithinRect(imageRect.size, viewFrame, _hAlign, _vAlign);
