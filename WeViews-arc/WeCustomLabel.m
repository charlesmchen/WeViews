...
+ (WeCustomLabel*) create:(NSString*) text
    ...
    WeCustomLabel* result = [[WeCustomLabel alloc] init];
    result.text = text;
    result.font = font;
...
- (CTTypesetterRef) createTypesetter {
    ...
    CTTypesetterRef typeSetter = CTTypesetterCreateWithAttributedString((__bridge CFAttributedStringRef) attributedText);
    return typeSetter;
}
...
- (CGSize) sizeThatFits:(CGSize) value {
    ...
    CGFloat lineHeight = ceilf(_font.leading * _lineHeightFactor);
    int numberOfLines = 0;
    ...
    while (index < [_text length]) {
        ...
        index += rawLineCount;
        numberOfLines++;
        if ((_maxNumberOfLines > 0) && (numberOfLines >= _maxNumberOfLines)) {
            break;
        }
    }
    ...
    while (index < [_text length]) {
        ...
        switch (_textAlignment) {
            ...
            case UITextAlignmentRight:
                ...
                break;
            default:
                _wv___FAIL(@"Unknown text alignment: %d", _textAlignment);
        }
