...
@interface WeActionSheet ()

@property (nonatomic, retain) WeActionSheet *selfReference;
...
+ (WeActionSheet*) create:(NSArray*) items
	...
	WeActionSheet* result = [[WeActionSheet alloc] init];
    result.items = items;
    result.title = title;
...
- (void) populate {
    self.sheet = [[UIActionSheet alloc] initWithTitle:_title
                                              delegate:self
                                     cancelButtonTitle:nil
                                destructiveButtonTitle:nil
                                     otherButtonTitles:nil];

    for (WeViewOption* item in _items) {
        item.customIndex = [_sheet addButtonWithTitle:item.label];
    }

    if (_cancelButtonTitle != nil) {
        int cancelIndex = [_sheet addButtonWithTitle:_cancelButtonTitle];
        [_sheet setCancelButtonIndex:cancelIndex];
    }
...
- (void) showFromBarButtonItem:(UIBarButtonItem*) item
    ...
    [self populate];
    if ([_sheet respondsToSelector:@selector(showFromBarButtonItem:animated:)]) {
        [_sheet showFromBarButtonItem:item
                            animated:animated];
    } else {
        [_sheet showFromToolbar:toolbar];
    }
    // Magic sauce.
    _selfReference = self;
}
...
clickedButtonAtIndex:(NSInteger) buttonIndex {
    ...
    [_dismissTarget performSelector:_dismissSelector];
...
- (void) dismissImmediately {
    [_sheet dismissWithClickedButtonIndex:-2
                                animated:NO];
