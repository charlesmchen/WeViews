...
- (id) init {
    ...
    _topMargin = _bottomMargin = _leftMargin = _rightMargin = 0;
    _spacing = 0;
    _touchMode = LAYER_TOUCH_MODE_ALWAYS;
...
+ (WePanelLayer*) create:(NSArray*) views
    ...
    WePanelLayer* result = [[WePanelLayer alloc] init];
...
- (WePanelLayer*) withTouchMode :(LayerTouchMode) value {
    self.touchMode = value;

    if (value == LAYER_TOUCH_MODE_DOWN) {
...
- (CGPoint) insetOrigin {
    return CGPointMake(self.leftMargin + _panel.borderWidth,
                       self.topMargin + _panel.borderWidth);
}

- (CGSize) insetSize {
    return CGSizeMake(self.leftMargin + self.rightMargin + 2 * _panel.borderWidth,
                      self.topMargin + self.bottomMargin + 2 * _panel.borderWidth);
}
...
- (WePanelLayer*) addView:(UIView*) value {
    NSMutableArray* result = [NSMutableArray arrayWithArray:_views];
    [result addObject:value];
    self.views = result;

    [_panel addSubview:value];
...
- (BOOL) containsView:(UIView*) value {
    return [_views containsObject:value];
}

- (BOOL) debugLayout {
    return _panel.debugLayout;
}
