...
- (void) addHandler:(id<IHandler>) handler
...
- (id<IHandler>) addSelectorHandler:(SEL) selector
                             target:(id) target
                           argument:(id) argument
                             events:(UIControlEvents) events {
    SelectorHandler* handler = [SelectorHandler create:target
                                              selector:selector
                                              argument:argument];
...
- (id<IHandler>) addClickSelector:(SEL) selector
                           target:(id) target
                         argument:(id) argument {
    return [self addSelectorHandler:selector
                             target:target
                           argument:argument
...
- (id<IHandler>) addClickSelector:(SEL) selector
                           target:(id) target {
    return [self addClickSelector:selector
                           target:target
                         argument:nil];
}
...
- (void) drawBorderInRect:(CGRect) borderRect {
    ...
    if ((borderRect.size.width <= 2 * _borderWidth) ||
        (borderRect.size.height <= 2 * _borderWidth)) {
...
- (void) set_borderColor:(UIColor *) value {
    _borderColor = value;
    [self setNeedsDisplay];
}

- (void) set_borderWidth:(int) value {
    _borderWidth = _wv_max(0, value);
    [self setNeedsDisplay];
}
