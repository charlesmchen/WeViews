...
+ (WeScrollView*) create:(UIView*) value
                   mode:(FScrollMode) mode {
...
+ (WeScrollView*) createVertical:(UIView*) value {
    return [self create:value
                   mode:SCROLL_MODE_VERTICAL];
}

+ (WeScrollView*) createHorizontal:(UIView*) value {
    return [self create:value
                   mode:SCROLL_MODE_HORIZONTAL];
}

+ (WeScrollView*) createBoth:(UIView*) value {
    return [self create:value
...
- (CGSize) sizeThatFits:(CGSize) value {
    switch (_mode) {
        ...
        case SCROLL_MODE_BOTH:
            return _minSize;
        default:
            _wv___FAIL(@"Unknown mode: %d", _mode);
    }
}
...
- (void) layoutContents {
    ...
    switch (_mode) {
        case SCROLL_MODE_HORIZONTAL: {
            ...
            size = [_content sizeThatFits:maxSize];
        ...
        case SCROLL_MODE_BOTH:
            size = [_content sizeThatFits:CGSizeZero];
            break;
        default:
            _wv___FAIL(@"Unknown mode: %d", _mode);
    }
    ...
    self.scrollEnabled = ((self.contentSize.width > self.frame.size.width) ||
                          (self.contentSize.height > self.frame.size.height));

    if (_debugLayout) {
        NSLog(@"[%@ %@] contentSize: %@, contentFrame: %@, self.frame: %@, scrollEnabled: %d",
              [self class],
              NSStringFromSelector(_cmd),
              FormatCGSize(self.contentSize),
              FormatCGRect(contentFrame),
