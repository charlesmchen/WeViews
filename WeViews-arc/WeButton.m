...
+ (WeButton*) createWithImage :(NSString*) upImageName {
    ...
    WeButton* result = [[WeButton alloc] init];

    UIImage* upImage = [WeViews loadImage:upImageName];
    [result setImage:upImage
...
+ (WeButton*) createWithImage:(NSString*) upImageName
    ...
    WeButton* result = [self createWithImage:upImageName];

    // TODO: This is inconsistent with the handler approach.
...
- (WeButton*) addClickHandler:(id<IHandler>) handler {
    if (_handlerGroup == nil) {
        self.handlerGroup = [HandlerGroup create];
        [self addTarget:_handlerGroup
                   action:@selector(fireEvent)
         forControlEvents:UIControlEventTouchUpInside];
    }
    [_handlerGroup addHandler:handler];
    return self;
}
...
- (WeButton*) addSimpleNotification :(NSString*) name {
    return [self addClickHandler:[SimpleNotificationHandler create:name]];
}
