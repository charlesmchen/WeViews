...
+ (WePanel*) create {
    WePanel* result = [[WePanel alloc] init];
    return result;
}
...
- (WePanel*) addToolbarBackground {
    UIToolbar* background = [[UIToolbar alloc] init];
...
- (CGSize) sizeThatFits:(CGSize) value {
    ...
    for (WePanelLayer* layer in _layers) {
...
//        }
    ...
    if (_maxSize.width >= 0) {
        result.width = _wv_min(result.width, _maxSize.width);
    }
    if (_maxSize.height >= 0) {
        result.height = _wv_min(result.height, _maxSize.height);
    }
...
- (WePanel*) setFixedWidth:(int) value {
    self.minSize = CGSizeMake(value, _minSize.height);
    self.maxSize = CGSizeMake(value, _maxSize.height);
    return self;
}

- (WePanel*) setFixedHeight:(int) value {
    self.minSize = CGSizeMake(_minSize.width, value);
    self.maxSize = CGSizeMake(_maxSize.width, value);
    return self;
}
