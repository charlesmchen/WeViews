...
@implementation WePanelLayoutEvenFillGrid
...
- (id) init {
    ...
    _cellSizeHint = CGSizeZero;

    return self;
}

+ (WePanelLayoutEvenFillGrid*) createWithColumnCount:(int) columnCount
                                       cellSizeHint:(CGSize) cellSizeHint {
    //                     gridMode:(GridLayerMode) gridMode {
    WePanelLayoutEvenFillGrid* result = [[WePanelLayoutEvenFillGrid alloc] init];
    result.columnCount = columnCount;
    result.cellSizeHint = cellSizeHint;
    //    result.gridMode = gridMode;
    return result;
}

+ (WePanelLayoutEvenFillGrid*) createWithRowCount:(int) rowCount
                                    cellSizeHint:(CGSize) cellSizeHint {
    //                     gridMode:(GridLayerMode) gridMode {
    WePanelLayoutEvenFillGrid* result = [[WePanelLayoutEvenFillGrid alloc] init];
...
- (LayoutMode) mode {
    return LAYOUT_MODE_EVEN_FILL_GRID;
}

- (CGSize) sizeThatFits:(CGSize) size
                  layer:(WePanelLayer*) layer {
    RowAndColumnCount rowAndColumnCount = [self rowAndColumnCount:layer];
    CGSize result = CGSizeMake(layer.spacing * (rowAndColumnCount.columnCount - 1) + _cellSizeHint.width * rowAndColumnCount.columnCount,
                               layer.spacing * (rowAndColumnCount.rowCount - 1) + _cellSizeHint.height * rowAndColumnCount.rowCount);
...
- (void) layoutContents:(CGSize) size
    ...
    if (layer.debugLayout) {
        NSLog(@"[%@ layoutContents] size %@", [self class], FormatSize(size));
    }
    ...
    for (UIView* item in layer.views) {
        int row = index / rowAndColumnCount.columnCount;
        int column = index % rowAndColumnCount.columnCount;
        index++;

        int x = left + column * cellWidth;
        if (rowAndColumnCount.columnCount > 1) {
            x += hSpacingTotal * column / (rowAndColumnCount.columnCount - 1);
        }
        int y = top + row * cellHeight;
        if (rowAndColumnCount.rowCount > 1) {
            y += vSpacingTotal * row / (rowAndColumnCount.rowCount - 1);
        }
